...
class Pledge_forum extends Pledge_Controller 
{
  var $session_array;
  ...
  public function index() 
  {
    ...
    $vars['lines_general'] = $this->file_functions->line_number_trim(SEPFILES_BASE_PATH . 'Forum/General/pledge_threads_log.html');
    $vars['last_general'] = $this->file_functions->last_line(SEPFILES_BASE_PATH . 'Forum/General/pledge_threads_log.html');
    $vars['lines_project'] = $this->file_functions->line_number_trim(SEPFILES_BASE_PATH . 'Forum/Project/pledge_threads_log.html');
    $vars['last_project'] = $this->file_functions->last_line(SEPFILES_BASE_PATH .'Forum/Project/pledge_threads_log.html');
    $vars['lines_misc'] = $this->file_functions->line_number_trim(SEPFILES_BASE_PATH . 'Forum/Misc/pledge_threads_log.html');
    $vars['last_misc'] = $this->file_functions->last_line(SEPFILES_BASE_PATH . 'Forum/Misc/pledge_threads_log.html');

    $headerData['js'] = array('pledge_bridge_main.js', 'threads.js');
    $headerData['css'] = array('style.css', 'internal.css');
    $headerData['title'] = "Discussion Board";
    
    $this->load->view('templates/header', $headerData);
    $this->load->view('pledge_thread_main_view', $vars);
  ...
  public function load_thread_view($dir = "")
  {
    ...
    if ($dir != "") 
    {
      ...
      {
        redirect('pledge_forum');
      }
      
      $vars['uploaded_by'] = $this->session_array['id'];
      $vars['privilege'] = $this->session_array['privilege'];
      $vars['content'] = $this->file_functions->load_file(SEPFILES_BASE_PATH . 'Forum/' . $folder . '/pledge_threads_log.html');
      $headerData['js'] = array('jQuery.js', 'jquery.min.js', 'threads.js');
      $headerData['css'] = array('style.css', 'internal.css');
      $headerData['title'] = "Forum";
      $this->load->view('templates/header', $headerData);
      $this->load->view('pledge_threads_view', $vars);
      $this->load->view('templates/footer');
    }
  }
  ...
  public function load_discussion_view($dir, $id)
  {
    ...
    if ($id) 
    {
      ...
      $logFilePath = SEPFILES_BASE_PATH . 'Forum/' . ucfirst($dir)  . "/pledge_threads_log_" . $id . ".html";
      ...
      $this->load->view('templates/header', $headerData);
      $this->load->view('pledge_discussion_view', $vars);
      $this->load->view('templates/footer');
    }
  }
  ...
  private function delete($source, $folder, $id)
  {
    ...
    {
      $this->file_functions->cutline(SEPFILES_BASE_PATH . 'Forum/' . $folder . '/pledge_threads_log_'. $id . '.html', $deleteid);
      redirect('pledge_forum/load_discussion_view/'.strtolower($folder).'/'.$id);
    }
  ...
  {    
    if ($this->input->post('post')) 
    {
      ...
      redirect('pledge_forum/load_discussion_view/'.$dir.'/'.$id);
    }    
  } 
  ...
  public function thread_action($dir)
  {
    ...
    if ($this->input->post('new_post'))
    {
      ...
      redirect('pledge_forum/load_thread_view/'.strtolower($directory));
    }
  }
  ...
  private function update_thread_log($dir) 
  {
    ...
    if (!empty ($message)) {
      ...
      $pledge_thread_logs_path = SEPFILES_BASE_PATH . 'Forum/' . $folder . '/pledge_threads_log.html';
      
      $new_identifier = $this->file_functions->line_number($pledge_thread_logs_path);
        $handle = fopen($pledge_thread_logs_path, 'a');
        $log = "<br><br><div uploaded_by='".$this->session_array['user_name']."' class=\"thread\" id='".($new_identifier+1)."' style=\"border-bottom: 1px dotted black;\"><b class=\"topic\"><a class=\"link\" identifier='".($new_identifier + 1)."' href=\"".DOMAIN_BASE_PATH."pledge_forum/post/".strtolower($folder)."/".($new_identifier+1)."\">".str_replace("
...
";
  ...
  private function update_discussion_log($post, $identifier, $folder, $uploaded_by) {
    ...
    $new_identifier = $this->file_functions->line_number(SEPFILES_BASE_PATH . 'Forum/' . $folder . '/pledge_threads_log_'.$identifier.'.html') + 1;
    
    $handle = fopen(SEPFILES_BASE_PATH. 'Forum/' . $folder ."/pledge_threads_log_".$identifier.".html", 'a');
...
";
  ...
  private function delete_post($folder, $id) {
    $this->file_functions->cutline(SEPFILES_BASE_PATH . 'Forum/' . $folder.'/pledge_threads_log.html', $id);
    unlink(SEPFILES_BASE_PATH . 'Forum/'.$folder.'/pledge_threads_log_'.$id.'.html');
    redirect('pledge_forum/load_thread_view/'.strtolower($directory));
  }
