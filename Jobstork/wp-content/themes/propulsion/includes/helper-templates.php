...
class avia_dynamic_template
{
	...
	*/
	...
	function hr($element)
	{
		...
		{
			case 'default': 		$output .= avia_flag(false, $extraClass); break;
			case 'default_small': 	$output .= avia_flag(false, 'supersmall '.$extraClass); break;
			case 'whitespace': 		$output .= "<div class='hr hr_invisible $extraClass'></div>"; break;
			case 'custom': 			$output .= avia_flag($element['saved_value'][0]['dynamic_hr_text'],$extraClass); break;
		}
	...
	*/
	function shop($element)
	{
		...
		$output .= "<div class='$extraClass container_wrap ".$avia_config['layout']." template-shop shop_columns_".$avia_config['shop_overview_column']."'>";
...
function avia_template_set_page_layout($post_type = '', $layout_string = false)
{
	...
	else
	{
		...
		//get the global page layout option set in your backend
	 	 $result = avia_get_option($post_type, 'sidebar_class|three : content_class|nine alpha : layout|sidebar_right');
